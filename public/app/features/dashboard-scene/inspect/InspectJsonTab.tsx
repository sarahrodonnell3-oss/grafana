...
export class InspectJsonTab extends SceneObjectBase<InspectJsonTabState> {
  ...
  private applyLayoutChange(jsonObj: unknown) {
    ...
    if (!(gridItem instanceof DashboardGridItem)) {
      getLogger('features.dashboard-scene').logError(
        new Error('Cannot update layout: panel parent is not a DashboardGridItem')
      );
      return;
    }
  ...
  private applyV1PanelChange(jsonObj: unknown, panel: VizPanel, dashboard: DashboardScene) {
    ...
    if (!(panel.parent instanceof DashboardGridItem)) {
      getLogger('features.dashboard-scene').logError(new Error('Cannot update state of panel'), {
        panel: String(panel),
        gridItem: String(gridItem),
      });
      return;
    }
