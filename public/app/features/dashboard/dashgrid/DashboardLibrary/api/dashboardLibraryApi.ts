...
export async function fetchCommunityDashboards(
  ...
  getLogger('features.dashboard').logWarning('Unexpected API response format from Grafana.com:', {
...
export async function fetchProvisionedDashboards(datasourceType: string): Promise<PluginDashboard[]> {
  try {
    ...
    getLogger('features.dashboard').logError(
      error instanceof Error ? error : new Error('Error loading provisioned dashboards')
    );
    return [];
  }
}
