...
export class DashboardModel implements TimeModel {
  ...
  *panelIterator() {
  ...
  on<T>(event: AppEvent<T>, callback: (payload?: T) => void) {
    getLogger('features.dashboard').logInfo('DashboardModel.on is deprecated use events.subscribe');
    this.events.on(event, callback);
  }

  /** @deprecated */
  off<T>(event: AppEvent<T>, callback: (payload?: T) => void) {
    getLogger('features.dashboard').logInfo('DashboardModel.off is deprecated');
