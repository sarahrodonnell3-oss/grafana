  ...
  return function (dispatch, abortController) {
    ...
    const dataObservable = from(
      ...
      catchError((err) => {
        ...
        getLogger('features.library-panels').logError(
          err instanceof Error ? err : new Error('Error fetching library panels:')
        );
...
export function deleteLibraryPanel(uid: string, args: SearchArgs) {
  return async function (dispatch: Dispatch<Action>) {
    try {
      ...
      getLogger('features.library-panels').logError(e instanceof Error ? e : new Error('Unknown error'));
    }
  };
}
