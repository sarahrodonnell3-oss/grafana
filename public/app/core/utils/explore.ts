...
export const safeStringifyValue = (value: unknown, space?: number) => {
  ...
  try {
    ...
    getLogger('core.utils.explore').logError(error instanceof Error ? error : new Error(String(error)));
  }
    ...
    for (let index = 0; index < queries.length; index++) {
      ...
      if (query.datasource) {
        try {
          ...
          getLogger('core.utils.explore').logError(
            new Error('One of the queries has a datasource that is no longer available and was removed.')
          );
          validDS = false;
        }
      }
